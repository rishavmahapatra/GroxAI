...
export default function RecentSessions() {
    ...
    const res = await fetch(`${url}/getRecentSession/${JSON.parse(localStorage.getItem("user")).email}`)
    const session = await res.json();
    setSessions(session);
}
useEffect(()=>{getRecentSession()},[])
  return (
        ...
        <div className="mb-10 flex items-end justify-between">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-zinc-600 dark:text-zinc-300">
              Pick up where you left off or review previous interview
              sessions.
            </p>
          </div>
        </div>
